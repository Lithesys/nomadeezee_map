...
The authenticated editor and release control loop are implemented. `/admin` provides a MapLibre editing canvas, searchable features, draggable MultiPolygon vertices, draggable label anchors, draft metadata editing, local/PostGIS geometry validation, immutable revision history, restore-as-new-draft, publish queueing, GitHub workflow dispatch (when configured), release status, and generation-checked rollback.

The public style still points at the active immutable PMTiles release. The repository includes a GitHub Actions builder for the first Vietnam release; it expects a versioned Vietnam OSM PBF, a layered world GeoJSON, and a glyph archive. Until that release is built and activated, the admin canvas uses its temporary context fallback and the public contract returns the bootstrap manifest. See [docs/admin-editor.md](docs/admin-editor.md), [docs/publishing.md](docs/publishing.md), and [docs/operations.md](docs/operations.md).
